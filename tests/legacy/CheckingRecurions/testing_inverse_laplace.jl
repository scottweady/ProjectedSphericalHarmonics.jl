...
Ninvf = 𝒩⁻¹(f, D)
Ninvf_exact = -4/ProjectedSphericalHarmonics.λlm(l, m)/(2*l+1)*(1/(l+m+1)*Plm_.(l+1, m, sqrt.(1 .- r.^2)) + 1/(l-m)*Plm_.(l-1, m, sqrt.(1 .- r.^2))) .* exp.(im * m * θ)
...
wNinvf_exact = -exp.(im * m * θ ) .* 4/ProjectedSphericalHarmonics.λlm(l, m)/(2*l+1) .*(
    ...
    2*(2l+1)/(2*l-1)/(2l+3) * Plm_.(l, m, sqrt.(1 .- r.^2)) +
    (l+m-1)/(2*l-1)/(l-m) * Plm_.(l-2, m, sqrt.(1 .- r.^2)) 
)
...
SNinvf_exact = -1/(2l+1) * exp.(im * m * θ ) .* (
    ...
    2*(2l+1)/(2*l-1)/(2*l+3) * Plm_.(l, m, sqrt.(1 .- r.^2)) +
    (l+m)/(2*l-1)/(l-m-1) * Plm_.(l-2, m, sqrt.(1 .- r.^2)) 
)
...
∂zSNinvf_exact = (r, θ) -> 1/2/(2l+1)*exp(im*(m-1)*θ)*((l-m+1)*Plm_(l+1, m-1, sqrt(1 - r^2))+(l+m)*Plm_(l-1, m-1, sqrt(1 - r^2)))
∂z̄SNinvf_exact = (r, θ) -> -1/2/(2l+1)*exp(im*(m+1)*θ)*(1/(l+m+2)*Plm_(l+1, m+1, sqrt(1 - r^2))+1/(l-m-1)*Plm_(l-1, m+1, sqrt(1 - r^2)))


∂z∂zSNinvf_exact = (r, θ) -> -(l+m)*(l-m+1)/4*exp(im*(m-2)*θ)*Plm_(l, m-2, sqrt(1 - r^2))
∂z̄∂z̄SNinvf_exact = (r, θ) -> -1/(l+m+2)/(l-m-1)/4*exp(im*(m+2)*θ)*Plm_(l, m+2, sqrt(1 - r^2))
∂z∂z̄SNinvf_exact = (r, θ) -> 1/4*exp(im*m*θ)*Plm_(l, m, sqrt(1 - r^2))
