...
∂z∂zSNinvf_exact = (r, θ) -> -3*l/(2l+1)/2*exp(im*(l-2)*θ)*(Plm_(l+1, l-2, sqrt(1 - r^2)))/sqrt(1-r^2)

∂z∂zSNinvf_exact = (r, θ) -> -l/(2l+1)/2*exp(im*(l-2)*θ)*( (2l+1)*Plm_(l, l-2, sqrt(1 - r^2)) + (2l-3)*2*(-l + 1)*Plm_(l-2, l-2, sqrt(1 - r^2))  )
