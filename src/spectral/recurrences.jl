...
function build_weight_operators(Lspan, Mspan)
  ...
  ib, jb = _select_invertible_subsystem(Lspan, Mspan, length(Lspan))
  W = _build_weight_matrix(Lspan, Mspan, length(Lspan), length(Mspan))
  Wqr = qr(W[ib, jb])
  Wqr = (W=Wqr, ib=ib, jb=jb)  # store the QR factors and index info for later use
